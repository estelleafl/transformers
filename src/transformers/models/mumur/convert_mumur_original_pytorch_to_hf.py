...
"""Convert MuMUR checkpoints from the original repository. URL: https://github.com/timojl/mumur."""
...
def get_mumur_config(model_name):
    text_config = MuMURTextConfig()
    vision_config = MuMURVisionConfig(patch_size=16)

    use_complex_transposed_convolution = True if "refined" in model_name else False
    reduce_dim = 16 if "rd16" in model_name else 64

    config = MuMURConfig.from_text_vision_configs(
        text_config,
        vision_config,
...
def convert_mumur_checkpoint(model_name, checkpoint_path, pytorch_dump_folder_path, push_to_hub):
    config = get_mumur_config(model_name)
    model = MuMURForImageSegmentation(config)
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="mumur-rd64",
        type=str,
        choices=["mumur-rd16", "mumur-rd64", "mumur-rd64-refined"],
        help=(
            "Name of the model. Supported models are: mumur-rd64, mumur-rd16 and mumur-rd64-refined (rd meaning"
            " reduce dimension)"
        ),
    )
    parser.add_argument(
        "--checkpoint_path",
        default="/Users/nielsrogge/Documents/MuMUR/clip_plus_rd64-uni.pth",
        type=str,
        help=(
    ...
    convert_mumur_checkpoint(args.model_name, args.checkpoint_path, args.pytorch_dump_folder_path, args.push_to_hub)
